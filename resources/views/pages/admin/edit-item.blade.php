...
@section('page_name2', 'Edit Barang')

@section('content_admin')
    <div class="content">
        <div class="card">
            <form id="form" action="{{ route('admin.item.update', $item->id) }}" method="POST"
                    ...
                    <div class="row">
                                    ...
                                    @foreach ($units as $key => $unit)
                                        <optgroup label="Satuan {{ $key }}">
                                            @foreach ($unit as $name)
                                                <option value="{{ $name }}" @selected($name == $item->unit)>
                                                    {{ $name }}
                                                </option>
                                            @endforeach
                                        </optgroup>
                        ...
                        <div class="col-12 mb-3 row">
                            ...
                            <div class="col-12 col-lg-9">
                                <textarea name="notes" id="notes" class="form-control" rows="3">{{ $item->notes }}</textarea>
